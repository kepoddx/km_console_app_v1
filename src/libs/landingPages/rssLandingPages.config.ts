...
export interface SitesMap {
    [key: string]: SiteMap | { [key: string]: string };
}
...
const sites: SiteRss[] = [
    ...
    {
        name: "greenville",
        feedUrl: "http://static.greenvilleonline.com/rss/",
        endPoints: [
            { endPoint: "sports", link: "http://rssfeeds.greenvilleonline.com/greenville/sports&x=1" }
        ],
        liveEndPoints: [
            { endPoint: "sports", link: "http://rssfeeds.greenvilleonline.com/greenville/sports&x=1" }
        ]
...
export default class RssLandingPagesConfig {

    getRssSiteMap$(): Observable<SitesMap> {
        return of(sitesMap)
    }

    getRssSites$(): Observable<SiteRss[]> {
        return of(sites)
    }

    getRssSiteByName$(siteName: string): Observable<SiteRss> {
        return of(sites)
            .pipe(
                map(sitesConfig => sitesConfig.filter(site => site.name === siteName)),
                map(siteArr => siteArr[0])
            )
    }

    getRssSiteEndPoint$(siteName: string, endPoint: string): Observable<any> {
        return of(sites)
            .pipe(
                map(sitesConfig => sitesConfig.filter(site => site.name === siteName)),
                map(siteArr => siteArr[0].liveEndPoints.filter(endpoint => endpoint.endPoint === endPoint))
            )
    }
}